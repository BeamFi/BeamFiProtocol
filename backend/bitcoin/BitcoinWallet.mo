...
import BitcoinType "BitcoinType";
...
module {
  ...
  type Utxo = BitcoinType.Utxo;
  type MillisatoshiPerByte = BitcoinType.MillisatoshiPerByte;
  let CURVE = BitcoinType.CURVE;
  ...
  public func send(network : Network, derivation_path : [[Nat8]], key_name : Text, dst_address : BitcoinAddress, amount : Satoshi, total_fee: Nat) : async [Nat8] {
    ...
    let tx_bytes = await build_transaction(own_public_key, own_address, own_utxos, dst_address, amount, total_fee);
    let transaction = BitcoinUtils.get_ok(Transaction.fromBytes(Iter.fromArray(tx_bytes)));
    ...
    let signed_transaction = BitcoinUtils.get_ok(Transaction.fromBytes(Iter.fromArray(signed_transaction_bytes)));
  ...
  // Calculate fees required for the BTC transaction, paid by the sender
  public func calc_fees(network: Network, derivation_path: [[Nat8]], key_name: Text, dst_address : BitcoinAddress, amount: Satoshi) : async Nat {
    ...
    let init_fee_per_byte : MillisatoshiPerByte = if(fee_percentiles.size() == 0) {
    ...
    let fee_per_byte_nat = Nat64.toNat(init_fee_per_byte);
    var total_fee : Nat = 0;
    let own_utxos = (await BitcoinApi.get_utxos(network, own_address)).utxos;

    loop {
      let transaction =
          BitcoinUtils.get_ok_except(Bitcoin.buildTransaction(2, own_utxos, [(#p2pkh dst_address, amount)], #p2pkh own_address, Nat64.fromNat(total_fee)), "Error building transaction.");
  ...
  func public_key_bytes_to_public_key(public_key_bytes : [Nat8]) : PublicKey {
      let point = BitcoinUtils.unwrap(Affine.fromBytes(public_key_bytes, CURVE));
      BitcoinUtils.get_ok(Publickey.decode(#point point))
  };
}
